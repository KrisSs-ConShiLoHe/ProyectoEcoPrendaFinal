...
class Fundacion(models.Model):
    ...
    def total_donaciones_recibidas(self):
        return Transaccion.objects.filter(fundacion=self, tipo__nombre_tipo='Donación').count()
...
class Transaccion(models.Model):
    ...
    def puede_aceptar(self, usuario):
        ...
        if self.user_destino:
            return usuario.id_usuario == self.user_destino.id_usuario
        return False
    ...
    def puede_rechazar(self, usuario):
        ...
        if self.es_donacion():
            return (usuario.es_representante_fundacion() and 
                    usuario.fundacion_asignada == self.fundacion)
        es_destino = (self.user_destino and 
                      usuario.id_usuario == self.user_destino.id_usuario)
        es_origen = (self.user_origen and usuario.id_usuario == self.user_origen.id_usuario)
